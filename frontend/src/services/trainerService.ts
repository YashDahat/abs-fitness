...
export const updateTrainer = async (trainerId: string, request: TrainerDto): Promise<TrainerDto> => {
  const response = await apiClient.put<TrainerDto>(`/api/v1/admin/trainers/${trainerId}`, request);
  return response.data;
};

export const deleteTrainer = async (trainerId: string): Promise<void> => {
  await apiClient.delete<void>(`/api/v1/admin/trainers/${trainerId}`);
};
